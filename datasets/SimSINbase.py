...
class SimSINBase(data.Dataset):
    def __init__(self,
                 ...
                 num_scales,
                 is_train=False):
        super(SimSINBase, self).__init__()
    ...
    def __getitem__(self, index):
        ...
        for i in self.frame_idxs:
            if i == "s":
                path =  os.path.join(self.data_path, folder[:-1] + self.rev_mapping[side] + '.png')
                inputs[("color", i, -1)] = self.get_color(path, do_flip)
            else:
                if not self.is_train:
                    path =  os.path.join(self.data_path, folder[:-5]+ '{0:03d}'.format(frame_index+1*i) + folder[-2:] +'.png')
                else:
                    path =  os.path.join(self.data_path, folder[:-5]+ '{0:03d}'.format(frame_index+1*i) + folder[-2:] +'.png')
                inputs[("color", i, -1)] = self.get_color(path, do_flip)
        ...
        if not self.is_train:
            path =  os.path.join(self.data_path, folder+'_depth.npy')
